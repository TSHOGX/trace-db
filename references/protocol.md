...
| `List` | Returns stable cursor-paginated session summaries with agent, cwd, time, model, provider, optional terminal status, fingerprint, and direct lineage metadata (`parentSessionId`, `parentRelation`, `subagentCount`). `cwdExact` avoids substring-prefix collisions; optional lineage collapse hides a child only when its parent is in the same filtered scope. |
| `Coverage` | Returns one session's fingerprint, archive commit time, event/source counts, and latest source mtime without loading its trace. |
