...
- **自适应回退**：连续无变化时周期退避至基础间隔的 4 倍；检测到活动后恢复基础间隔
- **批量提交**：同一 agent 的解析结果使用单事务提交，异常时自动退回逐 session 提交
- **流式解析**：Claude、Codex、Pi 的 JSONL 不会整体载入内存
- **一键安装**：自动生成并安装系统服务
...
5. **空扫描退避**：periodic 扫描无解析、无入库、无失败时按 1x → 2x → 4x 退避；有活动或失败时回到基础间隔
6. **失败隔离**：重复失败的 candidate 会进入持久 quarantine，内容指纹变化后自动重新尝试

这意味着即使每 30 分钟触发一次，如果没有变化，实际的计算开销非常小；连续空扫描还会逐步退避，最多放大到基础间隔的 4 倍。文件系统活动会立即触发一次扫描并重置退避。
