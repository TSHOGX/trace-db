//! Minimal FFI bindings to the pieces of the SQLite / FTS5 C ABI we need to
//! register a custom tokenizer.
//!
//! Approach (borrowed from Signal-FTS5-Extension, AGPL — reimplemented here):
//! a loadable extension's entry point receives a `*sqlite3_api_routines` table
//! rather than being able to call `sqlite3_*` symbols directly. So we describe
//! just enough of that table (by field ORDER — the layout is ABI, order is
//! load-bearing) to run `SELECT fts5(?1)` and pull out the `fts5_api` pointer,
//! then call `xCreateTokenizer` on it.
//!
//! Only the fields we actually call are typed precisely; everything else is a
//! same-sized `extern "C" fn()` placeholder purely to get the offsets right.
...
/// The tokenizer instance. Owns the jieba dictionary + stemmer (built once in
/// `xCreate`, reused across every `xTokenize`). Boxed and handed to FTS5 as an
/// opaque `*mut Fts5Tokenizer`.
pub struct Fts5Tokenizer {
    pub engine: crate::segment::Engine,
}

/// `xToken` callback FTS5 passes into `xTokenize`. Offsets are BYTE offsets
/// into the original UTF-8 input (see fts5.h).
...
/// `fts5_tokenizer` (v1) — the three function pointers FTS5 stores for a
...
pub struct Fts5TokenizerApi {
    ...
    pub x_delete: extern "C" fn(tokenizer: *mut Fts5Tokenizer),
...
/// `fts5_api` — only `iVersion` + `xCreateTokenizer` matter to us. The real
/// struct has more members after these, but we never call them, and since we
/// only ever hold a `&` and read the two leading fields, the trailing layout
/// is irrelevant.
#[repr(C)]
pub struct Fts5Api {
    pub i_version: c_int,
    pub x_create_tokenizer: extern "C" fn(
        ...
        p_context: *mut c_void,
        tokenizer: *mut Fts5TokenizerApi,
        x_destroy: extern "C" fn(module: *mut c_void),
    ) -> c_int,
}

/// `sqlite3_api_routines` (sqlite3ext.h). Field ORDER is the ABI contract.
/// Placeholders are untyped `extern "C" fn()` — never called, only spacing.
#[repr(C)]
pub struct Sqlite3ApiRoutines {
    ...
    pub finalize: extern "C" fn(stmt: *mut Sqlite3Stmt) -> c_int,
    _free: extern "C" fn(),
    ...
    pub libversion_number: extern "C" fn() -> c_int,
    _malloc: extern "C" fn(),
    _mprintf: extern "C" fn(),
    _open: extern "C" fn(),
    _open16: extern "C" fn(),
    pub prepare: extern "C" fn(
        db: *mut Sqlite3,
        query: *const c_uchar,
        query_len: c_int,
    ...
    pub step: extern "C" fn(stmt: *mut Sqlite3Stmt) -> c_int,
