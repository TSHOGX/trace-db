        ...
        Ok(Response::new(pb::ListResponse {
            sessions: page
                ...
                .map(|row| pb::SessionSummary {
                    ...
                    parent_relation: row.parent_relation.map(|value| value.to_string()),
...
fn session_to_proto(session: Session) -> pb::Session {
    pb::Session {
        ...
        parent_relation: session.parent_relation.map(|value| value.to_string()),
        fork_point_native_id: session.fork_point_native_id,
